...
class Triangle:
    ...
    def __init__(self, vertices : tuple[Point, Point, Point]) -> None:
        ...
        # Check cross product of vertices to check if the 3 given points are on a single line,
        # and thus making an invalid triangle
        if ((vertices[1].x - vertices[0].x) * (vertices[2].y - vertices[0].y)
    ...
    def get_circumcenter(self) -> Point:
        ...
        ux = (
            (1 / d) * (
                ...
                (self.vertices[1].y - self.vertices[2].y) + (self.vertices[1].x**2 + self.vertices[1].y**2) *
                (self.vertices[2].y - self.vertices[0].y) + (self.vertices[2].x**2 + self.vertices[2].y**2) *
        ...
        uy = (
            (1 / d) * (
                (self.vertices[0].x**2 + self.vertices[0].y**2) *
                (self.vertices[2].x - self.vertices[1].x) + (self.vertices[1].x**2 + self.vertices[1].y**2) *
                (self.vertices[0].x - self.vertices[2].x) + (self.vertices[2].x**2 + self.vertices[2].y**2) *
