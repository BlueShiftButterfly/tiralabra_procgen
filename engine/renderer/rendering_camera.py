...
class RenderingCamera:
    ...
    def screen_to_world_coordinates(self, screen_pos: Vector2) -> Vector2:
        ...
        world_coordinates = Vector2((screen_pos.x - screen_center.x) * self.total_render_scale + self.position.x, (screen_center.y-screen_pos.y) * self.total_render_scale + self.position.y)
        return world_coordinates

    def world_to_screen_coordinates(self, world_pos: Vector2) -> Vector2:
        screen_center = Vector2(self.__screen.get_width()/2, self.__screen.get_height()/2)
        screen_coordinates = Vector2(((world_pos.x - self.position.x) / self.total_render_scale) + screen_center.x, screen_center.y-((world_pos.y - self.position.y) / self.total_render_scale))   
